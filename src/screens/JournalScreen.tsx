...
export const JournalScreen: React.FC<{}> = () => {
  ...
  const renderCardEntry = (item: Entry, expense?: Expense, actionItem?: ActionItem) => (
    <TouchableOpacity 
      onLongPress={() => item.type !== 'system' && handleEditEntry(item)}
      onPress={() => isDesktop() && item.type !== 'system' && handleEditEntry(item)}
      delayLongPress={500}
      activeOpacity={0.9}
    >
  ...
  const renderMagazineEntry = (item: Entry, expense?: Expense, actionItem?: ActionItem) => (
    ...
    >
      ...
      {!isDesktop() && item.type !== 'system' && (
        <Text style={{ fontSize: 9, color: '#999', fontStyle: 'italic', marginTop: 4, paddingHorizontal: 16 }}>
          Long press to edit
        </Text>
      )}
    </View>
